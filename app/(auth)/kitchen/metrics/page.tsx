...
export default function KitchenMetricsPage() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      ...
      try {
        ...
        const stationsToFetch = selectedStation === 'all' ? stations : stations.filter(s => s.id === selectedStation)
        ...
        for (const station of stationsToFetch) {
          try {
            ...
            metricsData[station.id] = {
              ...
              efficiency: Math.min(100, Math.max(0, 100 - (avgPrepTime - 300) / 10)) // Efficiency score
            }
          } catch (error) {
            console.error(`Error fetching metrics for station ${station.id}:`, error)
            metricsData[station.id] = {
              avgPrepTime: 0,
              totalOrders: 0,
              completionRate: 0,
  ...
  if (stationCount > 0) {
    overallMetrics.avgPrepTime = Math.round(overallMetrics.avgPrepTime / stationCount)
    overallMetrics.completionRate = Math.round(overallMetrics.completionRate / stationCount * 100)
    overallMetrics.efficiency = Math.round(overallMetrics.efficiency / stationCount)
  }
  ...
  return (
    <ProtectedRoute roles={['cook', 'admin', 'demo']}>
                          ...
                          <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <div className="text-lg font-semibold">
                                  {Math.round(stationMetrics.completionRate * 100)}%
                                </div>
                              </div>
                              <div>
                                <div className="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            {stationMetrics.busyPeriods.length > 0 && (
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {stationMetrics.busyPeriods.map((period, index) => (
                    ...
                    <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-950 rounded">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-blue-600 dark:text-blue-400">
                          Consider pre-prepping common items during 11:30-13:00 rush period to reduce wait times.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 bg-green-50 dark:bg-green-950 rounded">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-green-600 dark:text-green-400">
                          Salad station is consistently meeting target prep times. Great work!
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 bg-yellow-50 dark:bg-yellow-950 rounded">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-yellow-600 dark:text-yellow-400">
                          Grill station prep times are 20% above target. Consider staff training or equipment check.
                        </div>
                      </div>
                    </div>
                  </div>
