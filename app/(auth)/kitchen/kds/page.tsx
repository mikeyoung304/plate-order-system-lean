...
export default function KDSPage() {
  ...
  const toggleSplitStation = (stationId: string) => {
    setSplitStations(prev => {
      ...
      } else if (prev.length < 4) { // Max 4 stations in split view
        ...
        {layoutMode === 'split' && !isFullscreen && (
          <div className="p-4 bg-gray-50 dark:bg-gray-800 border-b">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium">Select stations for split view:</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {stations.map(station => (
                <Button
                  key={station.id}
                  variant={splitStations.includes(station.id) ? "default" : "outline"}
                  size="sm"
                  onClick={() => toggleSplitStation(station.id)}
                  className="flex items-center gap-2"
                  disabled={!splitStations.includes(station.id) && splitStations.length >= 4}
                >
                  <div 
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: station.color }}
