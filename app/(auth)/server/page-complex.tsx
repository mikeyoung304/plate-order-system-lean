...
// // import { useConnection, useOrders, useServer, useTables } from '@/lib/state/domains' // Disabled to fix infinite re-render // Temporarily disabled to fix infinite re-render
...
export default function ServerPage() {
  // USE LEGACY HOOKS ONLY (domain contexts causing infinite re-render)
  const orderFlow = useOrderFlowState()
  const data = useServerPageData('default')
  const { toast } = useToast()
  
  // Extract data from working legacy hooks
  const tables = data.tables
  const residents = data.residents
  const orders = data.recentOrders
  const loading = { tables: data.loading, orders: data.loading }
  ...
  const handleSelectTable = (table: any) => {
    // Use legacy orderFlow (domain contexts disabled)
