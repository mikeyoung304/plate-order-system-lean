...
export default function KitchenPage() {
  ...
  const tableGroups = useMemo(() => {
    ...
    groups.forEach((tableOrders, tableKey) => {
      ...
      const latestTime = new Date(sortedOrders[sortedOrders.length - 1].created_at)
      const totalItems = sortedOrders.reduce((sum, order) => sum + (order.items?.length || 0), 0)
      const seatCount = new Set(sortedOrders.map(order => order.seat)).size
      
      // Determine status
      const newCount = sortedOrders.filter(o => o.status === 'new').length
      const preparingCount = sortedOrders.filter(o => o.status === 'in_progress').length
      const readyCount = sortedOrders.filter(o => o.status === 'ready').length
      
      let status: TableGroup['status'] = 'new'
      if (readyCount === sortedOrders.length) status = 'ready'
      else if (preparingCount > 0 || readyCount > 0) status = 'mixed'
      else if (newCount === sortedOrders.length) status = 'new'
      else status = 'preparing'
      
      // Calculate timing
      const maxElapsedMinutes = Math.max(...sortedOrders.map(order => {
    ...
    return tableGroups.sort((a, b) => a.earliestTime.getTime() - b.earliestTime.getTime())
  }, [orders])
  ...
  const filteredItems = useMemo(() => {
    if (viewMode === 'table') {
      return tableGroups.filter(group => {
        switch (filterBy) {
          case 'new': return group.status === 'new'
          case 'preparing': return group.status === 'preparing' || group.status === 'mixed'
          case 'ready': return group.status === 'ready'
          default: return true
        }
      })
    } else {
      return orders.filter(order => {
        switch (filterBy) {
          ...
          case 'preparing': return order.status === 'in_progress'
        ...
        <div className="container py-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400 mt-1">Manage orders and track preparation</p>
            </div>
                      ...
                      >
                        ...
                        <CardContent className="space-y-3">
                          {group.orders.map((order) => (
                            <div key={order.id} className="p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <span className="font-medium">Seat {order.seat}</span>
                                  {order.type === 'food' ? 
                          ...
                          <Button
                            ...
                            disabled={group.orders.every(o => o.status === 'ready')}
                          >
