...
export async function POST(request: NextRequest) {
    return measureApiCall('transcribe_api', async () => {
        ...
        if (!isAllowed) {
            return NextResponse.json(
                { error: 'Rate limit exceeded. Voice transcription is limited to 5 requests per minute.' },
                { 
                    status: 429,
                    headers: {
        ...
        const safeFileName = Security.sanitize.sanitizeIdentifier(audioFile.name || 'unknown');
        ...
        const safeTranscription = Security.sanitize.sanitizeHTML(transcription || '');
