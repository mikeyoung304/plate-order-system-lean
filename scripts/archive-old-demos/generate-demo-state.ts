...
class DemoStateGenerator {
  ...
  async run() {
    ...
    try {
      ...
      if (
        ...
        this.stations[0].id.includes('station-')
      ) {
        console.log(
          '   ⚠️  Skipping KDS queue generation (tables not available)'
        )
        console.log(
          '   ⚠️  Skipping metrics generation (KDS tables not available)'
        )
      } else {
    ...
    for (const table of this.tables) {
      ...
      if (isOccupied) {
        ...
        if (seats && seats.length > 0) {
          ...
          const occupiedSeatCount = Math.max(
            1,
            Math.floor(seats.length * (0.7 + Math.random() * 0.3))
          )
  ...
  async generateActiveOrders() {
    ...
    for (let i = 0; i < orderCount; i++) {
      ...
      const createdAt = getRealisticTimestamp(
        statusInfo.avgAge + Math.floor(Math.random() * 10)
      )
        ...
        orderType === 'food'
          ...
          : [
              MENU_ITEMS.beverages[
                Math.floor(Math.random() * MENU_ITEMS.beverages.length)
              ],
            ]
  ...
  async generateKDSQueue() {
    ...
    for (const order of orders) {
      ...
      for (const station of stations) {
        const routedAt = getRealisticTimestamp(
        ...
        if (isStarted) {
          routingData.started_at = getRealisticTimestamp(
            Math.floor(Math.random() * 10) + 2
          )
        }

        if (isCompleted) {
          routingData.completed_at = getRealisticTimestamp(
        ...
        await supabase.from('kds_order_routing').insert(routingData)
      }
    }
  ...
  getStationsForOrder(items: string[]): any[] {
    ...
    if (
      ...
      itemText.includes('beef') ||
      itemText.includes('burger') ||
      itemText.includes('steak')
    ) {
    ...
    if (
      itemText.includes('fries') ||
      itemText.includes('fried') ||
      itemText.includes('chips')
    ) {
    ...
    if (
      ...
      itemText.includes('vegetables') ||
      itemText.includes('fresh')
    ) {
  ...
  async generateMetrics() {
    ...
    for (
      let hour = Math.max(0, hoursToGenerate - 8);
      hour < hoursToGenerate;
      hour++
    ) {
      for (const station of this.stations) {
        ...
        for (let i = 0; i < metricCount; i++) {
          const actualPrepTime = Math.floor(
            prepTime + (Math.random() - 0.5) * variance
          )

          await supabase.from('kds_metrics').insert({
            station_id: station.id,
    ...
    console.log(
      `   ✓ Generated metrics for ${hoursToGenerate} hours across ${this.stations.length} stations`
    )
  }
}
