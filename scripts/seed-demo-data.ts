...
const DEMO_RESIDENTS = [
  ...
  },
  {
    ...
    favorite_items: ['Grilled chicken', 'Steamed vegetables', 'Unsweetened tea'],
    usual_table: 'Table 3',
    usual_seat: 2,
...
const SAMPLE_MENU_ITEMS = {
  breakfast: [
    'Scrambled eggs', 'French toast', 'Oatmeal with berries', 'Bacon', 'Sausage links',
    'Hash browns', 'Fresh fruit cup', 'Yogurt parfait', 'Pancakes', 'English muffin'
  ],
  lunch: [
    'Grilled chicken', 'Meatloaf', 'Fish and chips', 'Chicken soup', 'Caesar salad',
    'Turkey sandwich', 'Mac and cheese', 'Vegetable stir fry', 'Beef stew', 'Grilled cheese'
  ],
  dinner: [
    'Baked salmon', 'BBQ ribs', 'Roast beef', 'Chicken parmesan', 'Pork tenderloin',
    'Vegetable lasagna', 'Mashed potatoes', 'Green beans', 'Rice pilaf', 'Dinner roll'
  ],
  beverages: [
    'Coffee', 'Tea', 'Sweet tea', 'Unsweetened tea', 'Orange juice', 'Apple juice',
    'Milk', 'Water', 'Hot chocolate', 'Lemonade'
  ],
  desserts: [
    'Chocolate pudding', 'Vanilla ice cream', 'Apple pie', 'Cookies', 'Fresh fruit',
    'Jello', 'Cake slice', 'Sherbet', 'Pie slice', 'Brownies'
  ]
}
...
async function createDemoUsers() {
  ...
  for (const user of users) {
    try {
      ...
      if (profileError) {
        console.log(`Profile for ${user.email} may already exist:`, profileError.message)
      }
      ...
      if (roleError) {
        console.log(`Role for ${user.email} may already exist:`, roleError.message)
      }
...
async function createDemoTables() {
  ...
  const tables = [
    { label: 1, type: 'circle', status: 'available', position_x: 100, position_y: 100, seats: 4 },
    { label: 2, type: 'rectangle', status: 'available', position_x: 250, position_y: 100, seats: 6 },
    { label: 3, type: 'square', status: 'available', position_x: 400, position_y: 100, seats: 4 },
    { label: 4, type: 'circle', status: 'available', position_x: 100, position_y: 250, seats: 4 },
    { label: 5, type: 'rectangle', status: 'available', position_x: 250, position_y: 250, seats: 6 },
    { label: 6, type: 'circle', status: 'available', position_x: 400, position_y: 250, seats: 4 },
    { label: 7, type: 'square', status: 'available', position_x: 100, position_y: 400, seats: 4 },
    { label: 8, type: 'circle', status: 'available', position_x: 250, position_y: 400, seats: 4 }
  ]
  ...
  for (const table of tables) {
    ...
    if (seatsError) {
      console.error(`Failed to create seats for table ${table.label}:`, seatsError)
    } else {
...
async function createSampleOrders() {
  ...
  for (const resident of residents) {
    ...
    for (let i = 0; i < orderCount; i++) {
      ...
      const randomSeat = tableSeats[Math.floor(Math.random() * tableSeats.length)]
      ...
      for (let j = 0; j < favItemCount; j++) {
        orderItems.push(favoriteItems[Math.floor(Math.random() * favoriteItems.length)])
      }
      ...
      for (let j = 0; j < randomItemCount; j++) {
        orderItems.push(menuCategories[Math.floor(Math.random() * menuCategories.length)])
      }
      ...
      if (orderError) {
        console.error(`Failed to create order for ${resident.name}:`, orderError)
      }
    }
