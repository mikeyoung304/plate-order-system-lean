...
export function VoiceOrderPanel({ 
  ...
  const transcriptionDisplayText = useMemo(() => {
    ...
    if (voiceState.showConfirmation && voiceState.transcriptionItems.length > 0) {
      return voiceState.transcriptionItems.join(" • ");
    }
    if (voiceState.showConfirmation && voiceState.transcriptionItems.length === 0) return "No items detected.";
  ...
  const getButtonIcon = () => {
    ...
    if (voiceState.isRecording) return <Square className="h-6 w-6 text-red-500 fill-red-500" />;
