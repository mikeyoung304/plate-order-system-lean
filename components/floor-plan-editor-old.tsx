...
export function FloorPlanEditor({ floorPlanId }: FloorPlanEditorProps) {
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      ...
      // Convert to frontend format with layout positions
      const frontendTables: Table[] = floorPlanTables.map((table, index) => {
        // Calculate grid position for new tables
  ...
  const saveTables = useCallback(async (tablesToSave: Table[] = tables) => {
    ...
    try {
      ...
      // Convert tables to database format (core data only)
      const floorPlanTables = tablesToSave.map(table => ({
        id: table.id,
        label: table.label,
        type: table.type,
        seats: table.seats,
