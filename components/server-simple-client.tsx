...
type ServerSimpleClientComponentProps = {
  user: {
    id: string
    email?: string
  }
...
export function ServerSimpleClientComponent({
  user,
  profile,
}: ServerSimpleClientComponentProps) {
  ...
  const loadTables = useCallback(async () => {
    try {
      ...
      if (tablesError) {
        console.error('[ServerSimple] Tables query error:', tablesError)
      ...
      console.error('[ServerSimple] Load error:', err)
      setError(
  ...
  const createNewOrder = async (tableId: string, seatNumber: number) => {
    try {
      ...
      console.error(
        '[ServerSimple] Creating new order for table',
        tableId,
        'seat',
        seatNumber
      )
      ...
      alert(
        `Create new order for Table ${tables.find(t => t.id === tableId)?.label}, Seat ${seatNumber}`
      )
    } catch (err) {
