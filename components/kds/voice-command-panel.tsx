        ...
        switch (command.action) {
          ...
          case 'priority':
            if (command.target && command.value && onSetPriority) {
              const orderNumber = command.target
              const priority =
                typeof command.value === 'number' ? command.value : 5
              const order = orders.find(
                o =>
                  o.order?.id?.endsWith(orderNumber) ||
                  o.order?.id?.slice(-6) === orderNumber
              ...
              if (order) {
                ...
                setFeedbackMessage(
                  VOICE_FEEDBACK.priority(orderNumber, priority.toString())
                )
                toast({
                  title: 'Priority Updated',
                  description: VOICE_FEEDBACK.priority(
                    orderNumber,
                    priority.toString()
                  ),
                  duration: 2000,
            ...
            {lastCommand && (
              <div className='space-y-2'>
                ...
                <div className='text-sm bg-gray-50 dark:bg-gray-800 p-3 rounded'>
                  <div>
                    <strong>Said:</strong> "{lastCommand.originalText}"
                  </div>
                  <div>
                    <strong>Parsed as:</strong> {lastCommand.action}
                  </div>
                  {lastCommand.target && (
                    <div>
                      <strong>Target:</strong> {lastCommand.target}
                    </div>
                  )}
                  {lastCommand.value && (
                    <div>
                      <strong>Value:</strong> {lastCommand.value}
                    </div>
                  )}
                  <div>
