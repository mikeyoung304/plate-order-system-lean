...
export const CanvasOptimized = React.memo(function CanvasOptimized({
  ...
  const drawingOptions = useMemo(() => ({
    ...
    showTableDimensions,
    showTableStatus
  }), [isGridVisible, gridSize, showTableLabels, showTableSeats, showTableDimensions, showTableStatus])
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const cursorStyle = interactions.getCursorStyle(
      x, y, selectedTable, hoveredTable, tables, zoomLevel, e.ctrlKey || e.metaKey
    )
    canvas.style.cursor = cursorStyle
  }, [
    interactionMode, selectedTable, interactions, tables, onSetHoveredTable,
    zoomLevel, onTableUpdate
  ])
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (selectedTable) {
      if (interactions.findResizeHandleAtPosition(x, y, selectedTable, zoomLevel)) return
      if (interactions.findRotationHandleAtPosition(x, y, selectedTable, zoomLevel)) return
    }
    
    const clickedTable = interactions.findTableAtPosition(x, y, tables)
    onSelectTable(clickedTable?.id || null)
  }, [
    interactionMode, selectedTable, interactions, tables, onSelectTable, zoomLevel
  ])
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (selectedTable) {
      const resizeDir = interactions.findResizeHandleAtPosition(x, y, selectedTable, zoomLevel)
      if (resizeDir) {
      ...
      if (interactions.findRotationHandleAtPosition(x, y, selectedTable, zoomLevel)) {
    ...
    interactions, selectedTable, tables, zoomLevel, onSetInteractionMode,
    onSetResizeDirection, onSetResizeStart, onAddToUndoStack, onSetRotateStart,
    onSetInitialRotation, onSelectTable, onSetDragOffset, onSetPanStart
  ])
  ...
  const handleWheel = useMemo(
    ...
    }, 50), [zoomLevel, panOffset, onSetZoom, onSetPanOffset])
