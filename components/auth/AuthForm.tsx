...
export function AuthForm() {
  ...
  const handleServerStates = useCallback(() => {
    ...
    if (signUpState?.success) {
      actions.setStatus('success', "Check your email for the confirmation link.")
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 
        state.mode === 'signup' ? "Could not create account. Please try again." : "Invalid email or password."
    ...
    <div className="w-full space-y-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button 
          ...
          disabled={state.isRateLimited || isLoading}
        >
          {isLoading ? "Processing..." : state.mode === 'signup' ? 'Sign Up' : 'Sign In'}
        </Button>
      </form>
      ...
      <div className="text-center space-y-4">
        <Button 
          variant="outline" 
          onClick={() => actions.setMode(state.mode === 'signup' ? 'signin' : 'signup')}
          className="w-full"
          disabled={isLoading}
        >
