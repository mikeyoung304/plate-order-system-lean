...
export function AuthForm() {
  ...
  const validateAndSanitizeInputs = useCallback(
    (formData: FormData) => {
      ...
      const sanitizedEmail = Security.sanitize
        .sanitizeHTML(rawEmail?.trim() || '')
        .toLowerCase()
      const sanitizedName = Security.sanitize.sanitizeUserName(rawName || '')
      const sanitizedRole = ['server', 'cook', 'admin'].includes(rawRole)
        ? rawRole
        : 'server'
  ...
  const handleEmailChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const sanitizedValue = Security.sanitize
