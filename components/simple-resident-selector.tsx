...
import {
  type User as Resident,
  getAllResidents,
} from '@/lib/modassembly/supabase/database/users'
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const seatSuggestions = await getSeatResidentSuggestions(
          tableId,
          seatNumber,
          3
        )
        ...
        const timeSuggestions = await getTimeBasedResidentSuggestions(
          currentHour,
          2
        )
        const recentTimeResidents = timeSuggestions
          .filter(
            sug =>
              !recentSeatResidents.some(r => r.resident_id === sug.resident_id)
          )
          .map(sug => ({
            resident_id: sug.resident_id,
            name: sug.name,
