...
import { useHasRole, useRole } from '@/lib/modassembly/supabase/auth'
...
export function BetaNavigation() {
  ...
  const accessibleNavItems = navItems.filter(
    item => item.roles.includes(userRole || '') || isAdmin
  )

  const currentItem = accessibleNavItems.find(
    item =>
                  ...
                  <Link key={item.href} href={item.href}>
                    ...
                    >
                      ...
                      {item.isNew && (
                        <Badge
                          variant='secondary'
                          className='absolute -top-1 -right-1 bg-green-500 text-white text-xs px-1 scale-75'
                        >
                          NEW
                        </Badge>
                      )}
                      {item.isBeta && (
                        <Badge
                          variant='secondary'
                          className='absolute -top-1 -right-1 bg-orange-500 text-white text-xs px-1 scale-75'
                        >
                          BETA
                        </Badge>
                      )}
