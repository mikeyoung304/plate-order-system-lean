...
export function FloorPlanEditor({ floorPlanId }: FloorPlanEditorProps) {
    ...
    (type: Table['type']): Table => {
      ...
      return {
        ...
        x: selectors.snapToGrid
          ? Math.round(
              (100 + (tableCount % 5) * gridOffset) / selectors.gridSize
            ) * selectors.gridSize
  ...
  const handleBringToFront = useCallback(() => {
    ...
    const highestZIndex =
      Math.max(...selectors.tables.map(t => t.zIndex || 0)) + 1
    actions.addToUndoStack([...selectors.tables])
  ...
  const handleSendToBack = useCallback(() => {
    ...
    const lowestZIndex =
      Math.min(...selectors.tables.map(t => t.zIndex || 0)) - 1
    actions.addToUndoStack([...selectors.tables])
    actions.updateTable(selectors.selectedTable.id, { zIndex: lowestZIndex })
  }, [selectors.selectedTable, selectors.tables, actions])

  const handleUpdateTableProperty = useCallback(
    (property: keyof Table, value: any) => {
      if (!selectors.selectedTable) {
        return
      }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        const moveDistance = e.shiftKey
          ? selectors.snapToGrid
            ? selectors.gridSize
            : 10
          : 1
      ...
      <div className='grid grid-cols-1 gap-6'>
        ...
        <div className='flex flex-col lg:flex-row gap-6'>
          ...
          <SidePanel
            ...
            showTableStatus={selectors.showTableStatus}
            onToggleTooltips={() =>
              actions.setShowTooltips(!selectors.showTooltips)
            }
            onToggleLabels={() =>
              actions.setShowTableLabels(!selectors.showTableLabels)
            }
            onToggleSeats={() =>
              actions.setShowTableSeats(!selectors.showTableSeats)
            }
            onToggleDimensions={() =>
              actions.setShowTableDimensions(!selectors.showTableDimensions)
            }
            onToggleStatus={() =>
              actions.setShowTableStatus(!selectors.showTableStatus)
            }
          />
        </div>
      </div>
