...
export const mapBackendStatusToFrontend = (
  backendStatus: BackendTable['status']
): Table['status'] => {
  switch (backendStatus) {
    ...
    case 'reserved':
      return 'reserved'
    case 'out_of_service':
      return 'reserved' // Map OOS to reserved for now
...
export const mapFrontendStatusToBackend = (
  ...
  switch (frontendStatus) {
    ...
    case 'reserved':
      return 'reserved'
    case 'occupied':
      return 'reserved' // Map occupied to reserved for backend (adjust if backend adds 'occupied')
    default:
      return 'available'
  }
}
...
export const mapBackendTableToFrontend = (
...
export const mapFrontendTableToCreatePayload = (
  table: Table,
  floorPlanId: string
): TableCreatePayload => ({
  name: table.label,
...
export const mapFrontendTableToUpdatePayload = (
  frontendTable: Table,
  backendTable: BackendTable
): TableUpdatePayload | null => {
  ...
  if (frontendTable.label !== backendTable.name) {
    payload.name = frontendTable.label
    hasChanges = true
  }
  if (frontendTable.type !== backendTable.shape) {
    payload.shape = frontendTable.type
    hasChanges = true
  }
  if (Math.round(frontendTable.width) !== Math.round(backendTable.width)) {
    payload.width = frontendTable.width
    hasChanges = true
  }
  if (Math.round(frontendTable.height) !== Math.round(backendTable.height)) {
    payload.height = frontendTable.height
    hasChanges = true
  }
  if (Math.round(frontendTable.x) !== Math.round(backendTable.position_x)) {
    payload.position_x = frontendTable.x
    hasChanges = true
  }
  if (Math.round(frontendTable.y) !== Math.round(backendTable.position_y)) {
    payload.position_y = frontendTable.y
    hasChanges = true
  }
  if (
    Math.round(frontendTable.rotation || 0) !==
    Math.round(backendTable.rotation || 0)
  ) {
    payload.rotation = frontendTable.rotation || 0
    hasChanges = true
  }
  if (frontendTable.seats !== backendTable.seat_count) {
    payload.seat_count = frontendTable.seats
    hasChanges = true
  }
  const backendMappedStatus = mapFrontendStatusToBackend(
    frontendTable.status || 'available'
  )
  if (backendMappedStatus !== backendTable.status) {
    payload.status = backendMappedStatus
    hasChanges = true
  }
