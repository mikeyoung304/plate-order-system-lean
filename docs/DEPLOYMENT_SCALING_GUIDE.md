  ...
  "containerDefinitions": [
    {
      ...
      "secrets": [
        {"name": "SUPABASE_SERVICE_ROLE_KEY", "valueFrom": "arn:aws:secretsmanager:region:account:secret:plate/supabase-key"},
        {"name": "OPENAI_API_KEY", "valueFrom": "arn:aws:secretsmanager:region:account:secret:plate/openai-key"}
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
...
export { register, orderProcessingDuration, cacheHitRate, concurrentUsers, openaiCosts }
```
...
export const logPerformanceEvent = (metric: string, value: number, context?: any) => {
...
class CostMonitor {
  ...
  private monthlyBudget = parseInt(process.env.OPENAI_MONTHLY_BUDGET_CENTS || '10000')
...
class DisasterRecoveryManager {
  async initiateDRProcedure(incidentType: 'database' | 'application' | 'infrastructure') {
