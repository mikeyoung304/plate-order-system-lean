...
export interface CanvasInteractionsOptimized {
  ...
  findResizeHandleAtPosition: (x: number, y: number, selectedTable: Table | null, zoomLevel: number) => string | null
  findRotationHandleAtPosition: (x: number, y: number, selectedTable: Table | null, zoomLevel: number) => boolean
...
export function useCanvasInteractionsOptimized(
  ...
  const findTableAtPosition = useCallback((x: number, y: number, tables: Table[]): Table | null => {
    ...
    for (let i = tables.length - 1; i >= 0; i--) {
      ...
      if (table.type === "circle") {
        ...
        if ((localX - centerX) ** 2 + (localY - centerY) ** 2 <= radius ** 2) {
          return table
        }
      } else {
        if (localX >= table.x && localX <= table.x + table.width && 
            localY >= table.y && localY <= table.y + table.height) {
          return table
        }
      }
    }
  ...
  const findResizeHandleAtPosition = useCallback((
    ...
    for (const handle of handles) {
      if (localX >= handle.x - handleHitboxSize / 2 && 
  ...
  const findRotationHandleAtPosition = useCallback((
    ...
    const handleOffsetY_unrotated = -(table.height / 2) - rotationHandleLineOffset - (6 / zoomLevel)
  ...
  const getCursorStyle = useCallback((
    ...
    if (selectedTable) {
      const resizeDir = findResizeHandleAtPosition(x, y, selectedTable, zoomLevel)
      if (resizeDir) {
    ...
    return "default"
  }, [findResizeHandleAtPosition, findRotationHandleAtPosition, getStraightResizeCursor, getDiagonalResizeCursor])
