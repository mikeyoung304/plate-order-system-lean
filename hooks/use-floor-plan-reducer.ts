  ...
  switch (action.type) {
    ...
    case 'DELETE_TABLE':
      return {
        ...
        ui: {
          ...state.ui,
          selectedTableId: state.ui.selectedTableId === action.payload ? null : state.ui.selectedTableId
        },
        async: {
          ...state.async,
          unsavedChanges: true
    ...
    case 'ADD_TO_HISTORY': {
      const newStack = [...state.history.undoStack.slice(0, state.history.position + 1), [...action.payload]]
    ...
    case 'UNDO': {
      ...
      return {
        ...
        ui: {
          ...state.ui,
          selectedTableId: state.ui.selectedTableId && 
...
export const createSelectors = (state: FloorPlanState) => ({
  ...
  selectedTable: state.ui.selectedTableId ? 
    state.data.tables.find(t => t.id === state.ui.selectedTableId) || null : null,
...
export function useFloorPlanReducer(floorPlanId: string) {
  ...
  const [state, dispatch] = useReducer(floorPlanReducer, createInitialState(floorPlanId))
  ...
  const logger = useMemo(() => ({
    info: (message: string, ...args: any[]) => console.log(`[FloorPlanReducer] ${message}`, ...args),
    error: (message: string, ...args: any[]) => console.error(`[FloorPlanReducer] ${message}`, ...args),
    warning: (message: string, ...args: any[]) => console.warn(`[FloorPlanReducer] ${message}`, ...args),
  }), [])
  ...
  const actions = useMemo(() => ({
    ...
    setDragOffset: (offset: { x: number; y: number }) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { dragOffset: offset } }),
    setResizeDirection: (direction: string | null) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { resizeDirection: direction } }),
    setResizeStart: (start: { x: number; y: number }) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { resizeStart: start } }),
    setRotateStart: (start: number) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { rotateStart: start } }),
    setInitialRotation: (rotation: number) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { initialRotation: rotation } }),
    setPanStart: (start: { x: number; y: number }) => 
      dispatch({ type: 'SET_INTERACTION_STATE', payload: { panStart: start } }),
  ...
  const loadTables = useCallback(async () => {
    ...
    dispatch({ type: 'SET_ASYNC_STATE', payload: { isLoading: true, loadError: null } })
    ...
    try {
      ...
      dispatch({ type: 'SET_ASYNC_STATE', payload: { 
        isLoading: false, 
        unsavedChanges: false,
      ...
      dispatch({ type: 'SET_ASYNC_STATE', payload: { isLoading: false, loadError: errorMsg } })
  ...
  const saveTables = useCallback(async (): Promise<boolean> => {
    ...
    logger.info(`Saving ${state.data.tables.length} tables to floor plan ${floorPlanId}`)
