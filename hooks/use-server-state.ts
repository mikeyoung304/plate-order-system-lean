  ...
  switch (action.type) {
    ...
    case 'SELECT_SUGGESTION':
      return {
        ...state,
        selectedSuggestion:
          state.selectedSuggestion === action.suggestion
            ? null
            : action.suggestion,
      }
...
export function useServerState() {
  ...
  const actions = {
    ...
    setTables: useCallback(
      (tables: Table[]) => dispatch({ type: 'SET_TABLES', tables }),
      []
    ),

    setRecentOrders: useCallback(
      (orders: Order[]) => dispatch({ type: 'SET_RECENT_ORDERS', orders }),
      []
    ),

    setResidents: useCallback(
      (residents: Resident[]) => dispatch({ type: 'SET_RESIDENTS', residents }),
      []
    ),

    setUserData: useCallback(
      (userData: any) => dispatch({ type: 'SET_USER_DATA', userData }),
      []
    ),
    ...
    selectTable: useCallback(
      (table: Table) => dispatch({ type: 'SELECT_TABLE', table }),
      []
    ),

    selectSeat: useCallback(
      (seat: number) => dispatch({ type: 'SELECT_SEAT', seat }),
      []
    ),

    setOrderType: useCallback(
      (orderType: 'food' | 'drink') =>
        dispatch({ type: 'SET_ORDER_TYPE', orderType }),
      []
    ),

    selectResident: useCallback(
      (residentId: string) => dispatch({ type: 'SELECT_RESIDENT', residentId }),
      []
    ),

    setOrderSuggestions: useCallback(
      (suggestions: any[]) =>
    ...
    selectSuggestion: useCallback(
      (suggestion: any) => dispatch({ type: 'SELECT_SUGGESTION', suggestion }),
      []
    ),

    setView: useCallback(
      (view: ServerState['currentView']) =>
        dispatch({ type: 'SET_VIEW', view }),
      []
    ),

    toggleVoicePanel: useCallback(
      (show: boolean) => dispatch({ type: 'TOGGLE_VOICE_PANEL', show }),
      []
    ),
